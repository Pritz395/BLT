    ...
    try:
        ...
        if request.user.is_authenticated:
            try:
                ...
                completed_challenges_today = UserDailyChallenge.objects.filter(
                    user=request.user,
                ...
                last_checkin_for_timer = DailyStatusReport.objects.filter(user=request.user).order_by("-created").first()
